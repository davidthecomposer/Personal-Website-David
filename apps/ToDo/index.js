...
class Task {
    ...
    toggleAlarm(event, inputDate, inputTime, thisAlarm) {
        ...
        if (event.target.style.background.includes('off') || event.target.style.background === '') {
    ...
    handleAlarm(event) {
        ...
        let taskContainer = event.target.parentElement.parentElement;
        let taskExtras = event.target.parentElement.parentElement.nextElementSibling;
        ...
        if (toggleTheAlarm.style.background.includes('off') || toggleTheAlarm.style.background === '') {
            ...
            if (alarmOptions.style.display === 'none' || alarmOptions.style.display === '') {
    ...
    appendRow(taskData) {
        ...
        thisAlarmToggle.onclick = () => this.toggleAlarm(event, inputDate, inputTime, thisAlarm);
        inputDate.disabled = true;
        inputTime.disabled = true;
    }
...
const updateLocalInfo = () => {
    ...
    localStorage.clear();
     ...
     allTasks.forEach((task) => {
        ...
        let name = Number(allTasks[allTasks.indexOf(task)].classList.value.split(' ')[1]);
        let storageData = {
            thisName: name,
            circleStateOff: thisCircle.style.background.includes('empty') || thisCircle.style.background === '',
            taskName: thisInput.value,
            taskNotes: textArea.value,
            alarmStateOff: thisAlarmToggle.style.background.includes('off') || thisAlarmToggle.style.background === '',
