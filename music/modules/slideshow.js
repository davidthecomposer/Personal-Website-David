...
class SlideShow {
  constructor(showNumber) {
    ...
    this.slideToggleButton = this.slideShow.querySelector(
      ".slide-toggle-button"
    );
    this.slideEditInactive = this.slideShow.querySelector(
      ".slide-edit-inactive"
    );
  ...
  nextPane() {
    ...
    if (this.panes[activeIndex] === this.panes[lastIndex]) {
      ...
      this.slideText[lastIndex].classList.remove(
        this.textAnimations[lastIndex]
      );
      ...
      paneTick[lastIndex].setAttribute(
        "src",
        "images/slideshow/inactivePane.png"
      );
      paneTick[0].setAttribute("src", "images/slideshow/activePane.png");
      this.mainImages[lastIndex].classList.remove(
        this.slideAnimations[lastIndex]
      );
      ...
      this.slideText[activeIndex].classList.remove(
        this.textAnimations[activeIndex]
      );
      this.slideText[activeIndex + 1].classList.remove("invisible-text");
      this.slideText[activeIndex + 1].classList.add(
        this.textAnimations[activeIndex + 1]
      );
      paneTick[activeIndex].setAttribute(
        "src",
        "images/slideshow/inactivePane.png"
      );
      paneTick[activeIndex + 1].setAttribute(
        "src",
        "images/slideshow/activePane.png"
      );
      this.mainImages[activeIndex].classList.remove(
        this.slideAnimations[activeIndex]
      );
      this.mainImages[activeIndex].classList.add("invisible-panel");
      this.mainImages[activeIndex + 1].classList.remove("invisible-panel");
      this.mainImages[activeIndex + 1].classList.add(
        this.slideAnimations[activeIndex + 1]
      );
    }
  }
  ...
  autoPlayControl(event) {
    if (event === undefined) {
      ...
      event.target.getAttribute("src") === "images/slideshow/pause.png"
    ) {
